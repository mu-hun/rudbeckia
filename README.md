...
2. `스트리머`: `9`애서 `0`까지의 원소를 스택에 집어넣기 위한 반복문 블록의 시작입니다.
3. `브라우니 먹기 싫어 아 빵떡아`: `브라우니 싫어`는 현 스택의 맨위 원소 `10`에 1 을 뺀 값 `9`를 새 원소로 집어넣고, `빵떡아`는 스택 맨 위 원소인 `9`만큼
...
2. `글러먹은`: 피보나치 수 목록을 스택에 넣기 위한 반복문 블록의 시작입니다.
...
"한글 종성 문자"는 `ㄱ`,`ㄲ`, `ㄳ`, `ㄴ`, `ㄵ`, `ㄶ`, `ㄷ`, `ㄹ`, `ㄺ`, `ㄻ`, `ㄼ`, `ㄽ`, `ㄾ`, `ㄿ`, `ㅀ`, `ㅁ`, `ㅂ`, `ㅄ`, `ㅅ`, `ㅆ`, `ㅇ`, `ㅈ`, `ㅊ`, `ㅋ`, `ㅌ`, `ㅍ`, `ㅎ` 를 가리킵니다.
