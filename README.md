...
- 4번 스택은 트위치 아이디 `rudbeckia7` 에서 `rudbeckia` 가 _영원한 행복이라는_ 꽃말처럼 스택의 원소 할당을 불변으로 강제합니다. 넣을 수만 있고 뽑기가 불가능합니다.
...
- `난 트위치 최고 간땅이의 담력과 귀여움과 애교를 가진 연두라고 해`: 현재 스택의 모든 값을 드랍
