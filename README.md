...
`빵떡아` 예약어에 뒤따라 나오는 느낌표 갯수가 없으면 현재 위치한 스택의 원소 한개를 뽑고 나온 수 만큼 반복하고, 있으면 느낌표 갯수만큼 `글글글글` 과 `빵떡아` 사이에 입력된 명령어를 다시 반복합니다. `글글글글` 예약어의 별칭<sup>alias</sup>으로 `글러먹은` 또는 `스트리머` 가 있습니다.
...
따라서 위 코드는 `스트리머` 와 `빵떡아` 사이에 있는 `쪼아!!` 명령을 스택에서 뽑은 숫자 2만큼 반복을 합니다.
